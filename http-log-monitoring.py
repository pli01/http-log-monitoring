...
def display_summary_stats(now, stats_interval, stats_data):
    ...
    if len(stats_data):
       ...
       summary += '  total "sections" of the web site = {n} , most hits: {count}'.format(n=n, count=', '.join(count))
       ...
       for k in [ 'host', 'authuser', 'status', 'request_method' ]:
           ...
           summary += '  most common of {l} "{title}" : {count}'.format(title=k, l=l, count=', '.join(count))

    average = f_average(total_req_count,stats_interval)

    string = "{time} Summary stats last {count} seconds: hits = {total} - average = {average}/s - size = {size_bytes} bytes".format(time=display_time(now), total=total_req_count, count=stats_interval, average=average, size_bytes=size_bytes)
    ...
    while True:
        ...
        if line:
            ...
            if alarm_timer == alarm_interval:
                alert_sent = alarm_threshold(now, alarm_total_req_count, alarm_interval, threshold, alert_sent)
                alarm_total_req_count = 0
                alarm_timer = 0
