...
def alarm_threshold(now, alarm_total_req_count, alert_intervall, threshold, alert):
    """
    ...
    """
    average = f_average(alarm_total_req_count,alert_intervall)
...
def display_summary_stats(now, total_req_count, stats_interval, stats_data):
    """
    ...
    print("{time} Summary stats last {count} seconds: hits = {total} - average = {average}/s".format(time=display_time(now), total=total_req_count, count=stats_interval, average=average))
    ...
    while True:
        ...
        if line:
            ...
            if stats_timer == stats_interval:
                display_summary_stats(now, total_req_count, stats_interval, stats_data)
            ...
            if alarm_timer == alarm_interval:
                alert = alarm_threshold(now, alarm_total_req_count, alarm_interval, threshold, alert)
                alarm_total_req_count = 0
                alarm_timer = 0
